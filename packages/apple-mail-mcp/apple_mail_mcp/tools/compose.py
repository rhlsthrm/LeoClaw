...
def reply_to_email(
    ...
    if send:
        ...
        # For draft, paste reply text into the compose window via clipboard
        # and save with Cmd+S. This preserves Mail's native HTML quoted-reply
        # formatting, unlike `set content` which overwrites the HTML layer.
        # Requires Accessibility permissions for the running process.
        set_content_script = f'''activate
                delay 0.5
        ...
        send_or_draft_command = '''tell application "System Events"
                    tell process "Mail"
                        delay 0.5
